...
func init() {
	PacketHandlers["clientsetting"] = func(c collections.Client, p *packetbuilders.Packet) {
		// 2 = mouse buttons
		// 0 = camera angle manual/auto
		// 3 = soundFX (false=on, wtf)
		c.Player().SetClientSetting(int(p.ReadByte()), p.ReadBool())
	}
	PacketHandlers["privacysettings"] = func(c collections.Client, p *packetbuilders.Packet) {
